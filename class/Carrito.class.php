...
class Carrito{
    ...
    public function add($articulo = array()){
        ...
        if(!empty($this->carrito)){
            $existe=0;
            foreach ($this->carrito as $row){
                if($row["unique_id"] === $unique_id){ $existe=1; } //si se repite 
            }
            if($existe==1){ unset($_SESSION["carrito"][$unique_id]); //elimino
            }else{  //sino add al carrito
        ...
        }else{//si carro esta vacio inserto producto al carro
           $_SESSION["carrito"][$unique_id] = $articulo;
        }
